...
/*	ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
					"connect_create_pre: last='%s', pos='%s', start='%s', end='%s'",
					b->last, b->pos, b->start, b->end);
*/
    /* connect variables */
...
/*    ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
            "connect_create: addr_text='%s' ",
            addr_text->data);

	ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
				"connect_create: uri='%s' host='%s' len=%d",
				aacf->connect_url->uri.data, aacf->connect_url->host.data,
...
*/
    ...
    hl = ngx_rtmp_netcall_http_format_request(NGX_RTMP_NETCALL_HTTP_POST,  &aacf->connect_url->host,
                                                &aacf->connect_url->uri, NULL, body, pool,
...
/*
	...
	for (; pc; pc = pc->next)
	{
		b = pc->buf;
		ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
						"connect_create: last='%s', pos='%s', start='%s', end='%s'",
						b->last, b->pos, b->start, b->end);
	}
*/
    return hl;
...
ngx_rtmp_authen_connect(ngx_rtmp_session_t *s, ngx_rtmp_connect_t *v)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
            "connect (authen): app='%s' args='%s' flashver='%s' swf_url='%s' "
...
/*
    ngx_log_error(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
            "connect (authen) 2: app='%s' args='%s' flashver='%s' swf_url='%s' "
            "tc_url='%s' page_url='%s' ",
            s->app.data, s->args.data, s->flashver.data, s->swf_url.data, s->tc_url.data, s->page_url.data);
*/
